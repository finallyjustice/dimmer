...
float weight[NUM_MACH][NUM_HOUR];
...
char* fgetcsvline(FILE *f)
{
	...
	if(ret_stat != NULL)
	{
		...
		for(long c=start; c<end; c+=step)
		{
			...
			// I want to compute the weight here
			long bs = idx*step+base;
			long bt = c;
			...
			midx = machines[mid];
			mem[midx][idx] += musage*w;
			cpu[midx][idx] += cusage*w;
			ucache[midx][idx] += uusage*w;
			tcache[midx][idx] += tusage*w;
			//weight[midx][idx] += w;
		}
	}
